...
function* splitEveryTwo<T>(array: T[]): Generator<T[], void, void> {
    let buffer = [];
...
class Gtp {
    ...
    inputDom: HTMLFormElement;
    outputDom: HTMLTextAreaElement;
    LzAnalyzeHandler: ((result: any[]) => void) | null;
    kataAnalyzeHandler: ((result: KataInfo) => void) | null;
    resolve: ((line: string) => void) | null;
    reject: ((line: string) => void) | null;
    constructor() {
        this.inputDom = document.getElementById("input") as HTMLFormElement;
        ...
        this.outputDom.addEventListener("message", event => {
            ...
            this.process(target.value);
            target.value = "";
        }, false);
        this.LzAnalyzeHandler = null;
        this.kataAnalyzeHandler = null;
        this.resolve = null;
        this.reject = null;
    }
    ...
    _command(str: string) {
        ...
        this.inputDom.dispatchEvent(new CustomEvent('submit', { cancelable: true }));
    }

    command(str: string): Promise<string> {
    ...
    lzAnalyze(interval: number, handler: ((result: any[]) => void) | null) {
        if (this.inputDom.command.getAttribute("disabled") != null) {
            return;
        }
    ...
    kataAnalyze(interval: number, handler: ((result: KataInfo) => void) | null) {
        if (this.inputDom.command.getAttribute("disabled") != null) {
            return;
        }
    ...
    parseLzAnalyze(line: string) {
        ...
        infos.shift(); // remove the first empty string
        const result = [];
        for (const info of infos) {
            const parsed: { [key: string]: any } = {};
            const parts = info.split("pv ");
            for (const pair of splitEveryTwo(parts[0].split(/\s/))) {
                switch (pair[0]) {
                case "visits":
                case "order":
                    parsed[pair[0]] = parseInt(pair[1]);
                    break;
                case "winrate":
                case "prior":
                case "lcb":
                    parsed[pair[0]] = parseInt(pair[1]) / 10000;
                    break;
                default:
                    parsed[pair[0]] = pair[1];
                }
            }
            const pv = parts[1].split(/\s/);
            parsed[pv.shift()!] = pv;
            result.push(parsed);
        }
    ...
    parseKataAnalyze(line: string) {
        ...
        const parts = line.split("ownership ");
        const result: KataInfo = {
            info: []
        };
        const infos = parts[0].split("info ");
        infos.shift(); // remove the first empty string
        for (const info of infos) {
            const parsed: { [key: string]: any } = {};
            const subparts = info.split("pv ");
            for (const pair of splitEveryTwo(subparts[0].split(/\s/))) {
                switch (pair[0]) {
                case "visits":
                case "order":
                    parsed[pair[0]] = parseInt(pair[1]);
            ...
            const pv = subparts[1].split(/\s+/);
            parsed[pv.shift()!] = pv;
            result.info.push(parsed);
        }
        if (parts.length === 2) {
            const tokens = parts[1].split(/\s+/);
            tokens.pop();
            result.ownership = tokens.map(parseFloat);
        }
